...
import { ArrowLeft, ExternalLink, Lightbulb, Zap, Target, Shield, Database, Rocket, Download } from "lucide-react";
...
export default function CaseStudyContent({ study }: { study: CaseStudy }) {
  return (
    <main className="min-h-screen bg-[#0a0a0b] text-white">
      ...
      {/* Hero */}
      <section className="pt-32 pb-16 bg-gradient-to-r from-indigo-600/20 to-purple-600/20 border-b border-indigo-500/30">
        <motion.div
        ...
        >
          ...
          <p className="text-lg md:text-xl text-[var(--color-secondary)] mt-5 max-w-3xl mx-auto">
            {study.tagline}
          </p>
      ...
      <Section title="Overview">
        <p className="text-[var(--color-secondary)] leading-relaxed">{study.overview}</p>
      </Section>
      ...
      <Section title="Problems" bg>
        ...
        <p className="text-[var(--color-secondary)] mt-4 italic">{study.problemSummary}</p>
      </Section>
      ...
      <Section title="Solutions">
        <ul className="space-y-3">
          {study.solutions.map((s, i) => (
            <li key={i} className="flex items-start gap-3 text-[var(--color-secondary)]">
      ...
      <Section title="Key Features" bg>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {study.features.map((f, i) => (
            ...
            >
              <Target size={20} className="text-[var(--color-accent-light)] mb-3" />
